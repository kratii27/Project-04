...
@WebServlet(name = "TicketCategoryListCtl", urlPatterns = { "/ctl/TicketCategoryListCtl" })
public class TicketCategoryListCtl extends BaseCtl {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            List<TicketCategoryBean> list = model.search(bean, pageNo, pageSize);
            List<TicketCategoryBean> next = model.search(bean, pageNo + 1, pageSize);
        ...
        try {
            ...
            if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
                ...
                if (ids != null && ids.length > 0) {
                    TicketCategoryBean deleteBean = new TicketCategoryBean();
                    for (String id : ids) {
                        deleteBean.setId(DataUtility.getInt(id));
                        model.delete(deleteBean);
                        ServletUtility.setSuccessMessage("Ticket Category deleted successfully", request);
                    }
                } else {
    ...
    protected String getView() {
        return ORSView.TICKET_CATEGORY_LIST_VIEW;
    }
}