...
@WebServlet(name = "MusicPlaylistListCtl", urlPatterns = { "/ctl/MusicPlaylistListCtl" })
public class MusicPlaylistListCtl extends BaseCtl {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            List<MusicPlaylistBean> list = model.search(bean, pageNo, pageSize);
            List<MusicPlaylistBean> next = model.search(bean, pageNo + 1, pageSize);
        ...
        try {
            ...
            if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
                ...
                if (ids != null && ids.length > 0) {
                    MusicPlaylistBean deleteBean = new MusicPlaylistBean();
                    for (String id : ids) {
                        deleteBean.setId(DataUtility.getInt(id));
                        model.delete(deleteBean);
                        ServletUtility.setSuccessMessage("Music Playlist deleted successfully", request);
                    }
                } else {
    ...
    protected String getView() {
        return ORSView.MUSIC_PLAYLIST_LIST_VIEW;
    }
}